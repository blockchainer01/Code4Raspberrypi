...
#define S_FUNCTION_NAME  write_data_node1
...
static unsigned int count_send=0; 
static float data_send[65535]={0};
static unsigned int time_stamp_send[65535]={0};
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetInputPortWidth(S, 0, 2); // yy u=[stamp data_send]
    ssSetInputPortWidth(S, 1, 2); // yy u=[date num_index]
    ssSetInputPortDataType(S, 0, SS_DOUBLE);//Ä¬ÈÏŸÍÊÇdouble
...
static void mdlInitializeConditions(SimStruct *S)
{
    ...
    sprintf(dir_txt, "/home/pi/data_ras1_recvFrom_%d_%d.txt ", txttmp[0],txttmp[1]);
...
{      
  	const real_T *u  =ssGetInputPortRealSignal(S,0); // u=[stamp data_send]
    InputRealPtrsType uPtrs1 = ssGetInputPortSignalPtrs(S,1);//ssGetInputPortSignalPtrs(S,1);

    time_stamp_send[count_send] = u[0];
    data_send[count_send] = u[1];
    count_send++;
